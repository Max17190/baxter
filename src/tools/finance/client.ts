...
const log = createChildLogger("finance-client");
const BASE_URL = "https://api.financialdatasets.ai";
const breaker = new CircuitBreaker({ name: "financial-datasets", failureThreshold: 5, resetTimeoutMs: 60_000 });
...
export class FinancialDatasetsClient {
  ...
  async get<T>(path: string, params?: Record<string, string>): Promise<T> {
    return breaker.execute(async () => {
      ...
      log.debug({ path, params }, "API request");
      ...
      if (!response.ok) {
        ...
        log.warn({ path, status: response.status }, "API error");
        throw new Error(
